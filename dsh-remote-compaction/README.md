...
The plugin therefore validates and classifies the remote response but never converts opaque items into a fake text summary:
...
Runtime peer dependencies are pinned to DSH `0.1.0-rc.5`, based on `deepseek-harness@47f943859bef60e4160492346772ded9b24f765a`. `@deepseek-ai/dsh-compaction-basic` is intentionally pinned exactly because the protected hook is not a public provider seam.

The npm registry does not currently contain `@deepseek-ai/dsh-compaction-basic@0.1.0-rc.5` or the matching definition packages. This repository uses published `0.1.0-rc.6` packages only as a build and Loader test host after checking the hook signature against the rc.5 source. A real rc.5 tarball/profile install cannot be completed until the exact artifacts or a private registry are supplied. Do not widen the runtime peer range to rc.6 without repeating the full transaction and Loader suite.

## Configuration

The bundle patch disables the existing `compaction-basic` row before inserting this provider. Required and remote-specific fields are:

| Field | Default | Meaning |
| --- | --- | --- |
| `mode` | `auto` | `auto` permits Basic fallback; `remote-only` never falls back; `disabled` uses Basic directly. |
...
HTTP classification uses status codes rather than error text: 401 is authentication; 403 is permission; 404/405/501 are unsupported; 408/409/429/5xx are temporarily unavailable; other non-success statuses are invalid requests. Transport, timeout, malformed JSON, schema validation, size bounds, and caller abort remain distinct.
...
Default tests use local fetch mocks. The Loader suite boots YAML through the real Cordis Loader, checks the service and invariant composition, exercises remote-only and inherited fallback policy, and verifies disposal removes contributed services.
